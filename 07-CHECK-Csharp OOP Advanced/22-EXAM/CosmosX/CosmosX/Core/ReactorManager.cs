...
namespace CosmosX.Core
{
    public class ReactorManager : IManager
    {
        ...
        public string ModuleCommand(IList<string> arguments)
        {
            ...
            {
                case "CryogenRod":
                    IEnergyModule energyModule = new CryogenRod(this.currentId, additionalParameter);

                    this.reactors[reactorId].AddEnergyModule(energyModule);
                    this.identifiableObjects.Add(energyModule.Id, energyModule);
                    this.modules.Add(energyModule.Id, energyModule);
                    break;
                case "HeatProcessor":
                    ...
                    break;
                case "CooldownSystem":
                    IAbsorbingModule absorbingModule2 = new CooldownSystem(this.currentId, additionalParameter);

                    this.reactors[reactorId].AddAbsorbingModule(absorbingModule2);
                    this.identifiableObjects.Add(absorbingModule2.Id, absorbingModule2);
                    this.modules.Add(absorbingModule2.Id, absorbingModule2);
        ...
        public string ExitCommand(IList<string> arguments)
        {
            ...
            long energyModulesCount = this.modules
                .Values
                .Count(m => m.GetType().Name == nameof(CryogenRod));

            long absorbingModulesCount = this.modules
                .Values
                .Count(m => m.GetType().Name == nameof(HeatProcessor));

            absorbingModulesCount += this.modules
                .Values
                .Count(m => m.GetType().Name == nameof(CooldownSystem));
